...
const OTHER_CATEGORIES = [
  { id: 'general', label: 'General Products', icon: Package, subs: ['General Household Products', 'General Consumer Products', 'Miscellaneous Products', 'Everyday Essentials', 'Utility Products', 'Replacement Products', 'Accessories', 'Specialty Products'] },
  { id: 'gifts', label: 'Gifts & Novelty', icon: Gift, subs: ['Gift Items', 'Gift Sets', 'Souvenirs', 'Novelty Items', 'Party Gifts', 'Celebration Items', 'Keepsakes', 'Personalized Gifts'] },
  { id: 'travel', label: 'Travel & Lifestyle', icon: Briefcase, subs: ['Travel Accessories', 'Luggage Accessories', 'Travel Organizers', 'Travel Bottles & Containers', 'Passport Holders', 'Travel Cases', 'Lifestyle Accessories'] },
  { id: 'cleaning', label: 'Cleaning & Maintenance', icon: Trash2, subs: ['Cleaning Tools', 'Cleaning Accessories', 'Storage & Organization', 'Maintenance Products', 'Household Maintenance Supplies', 'Utility Cleaning Products'] },
  { id: 'electronics', label: 'General Electronics & Accessories', icon: Zap, subs: ['Electronic Accessories', 'Cables', 'Adapters', 'Chargers', 'Batteries', 'Electronic Components', 'Small Electronic Devices', 'Device Accessories'] },
  { id: 'parts', label: 'Replacement Parts', icon: Wrench, subs: ['Appliance Parts', 'Furniture Parts', 'Electronic Parts', 'Automotive Accessories', 'Machine Parts', 'Replacement Hardware', 'Replacement Components'] },
  { id: 'promotional', label: 'Business & Promotional Products', icon: Tag, subs: ['Promotional Items', 'Branded Products', 'Business Gifts', 'Custom Merchandise', 'Event Merchandise', 'Promotional Accessories'] },
  { id: 'events', label: 'Events & Party Supplies', icon: Gift, subs: ['Party Decorations', 'Event Accessories', 'Celebration Supplies', 'Balloons', 'Party Tableware', 'Event Props', 'Event Gift Items'] },
  { id: 'novelty', label: 'Novelty & Fun Products', icon: Smile, subs: ['Toys & Novelty Items', 'Collectible Accessories', 'Fun Gadgets', 'Desk Toys', 'Hobby Accessories', 'Novelty Gifts'] },
  { id: 'bags', label: 'Bags & General Accessories', icon: ShoppingBag, subs: ['Shopping Bags', 'Storage Bags', 'Pouches', 'Cases', 'Organizers', 'General Accessories'] },
  { id: 'home', label: 'Miscellaneous Home Products', icon: Home, subs: ['Household Accessories', 'Home Utility Products', 'Decorative Accessories', 'Organization Products', 'General Home Items'] },
  { id: 'personal', label: 'Personal Accessories', icon: User, subs: ['Personal Utility Products', 'Personal Organizers', 'Everyday Accessories', 'Lifestyle Accessories', 'Travel Accessories'] },
  { id: 'packaging', label: 'Packaging Products', icon: Package, subs: ['Boxes', 'Bags', 'Wrapping Materials', 'Packaging Accessories', 'Labels', 'Protective Packaging'] },
  { id: 'misc', label: 'Specialty & Miscellaneous', icon: MoreHorizontal, subs: ['Specialty Products', 'Hard-to-Classify Products', 'Miscellaneous Physical Products', 'Other Products'] }
]

const COMMON_OPTION_TYPES = ['Color', 'Size', 'Material', 'Style', 'Type']
...
export default function OtherProductBuilder({ productType }: { productType: string | null }) {
  ...
  const [state, formAction, isPending] = useActionState(createOtherProductAction, null)
  ...
  const { isRestoring, clearDraft } = useDraftAutoSave('other', draftState, (data) => {
  ...
  const activeCategory = OTHER_CATEGORIES.find(c => c.id === category)

  const renderDynamicSpecs = () => {
    const lowerSub = subCategory.toLowerCase()

    if (lowerSub.includes('part') || lowerSub.includes('hardware') || lowerSub.includes('component')) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Compatible Models / Brand</label><input type="text" value={attributes.Compatibility || ''} onChange={e => handleAttributeChange('Compatibility', e.target.value)} placeholder="e.g. Universal, Fits XYZ Model" className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Part Number (Optional)</label><input type="text" value={attributes.PartNumber || ''} onChange={e => handleAttributeChange('PartNumber', e.target.value)} className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Material / Finish</label><input type="text" value={attributes.Material || ''} onChange={e => handleAttributeChange('Material', e.target.value)} className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Dimensions / Size</label><input type="text" value={attributes.Size || ''} onChange={e => handleAttributeChange('Size', e.target.value)} className="w-full border rounded-md px-3 py-2" /></div>
        </div>
      )
    }
    
    if (lowerSub.includes('electronic') || lowerSub.includes('cable') || lowerSub.includes('charger') || lowerSub.includes('batter')) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Connection Type / Specs</label><input type="text" value={attributes.Specs || ''} onChange={e => handleAttributeChange('Specs', e.target.value)} placeholder="e.g. USB-C, AA, 12V" className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Length / Capacity</label><input type="text" value={attributes.Capacity || ''} onChange={e => handleAttributeChange('Capacity', e.target.value)} placeholder="e.g. 1m, 10000mAh" className="w-full border rounded-md px-3 py-2" /></div>
    ...
    if (lowerSub.includes('packag') || lowerSub.includes('box') || lowerSub.includes('bag') || lowerSub.includes('container')) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm font-medium mb-1">Dimensions (L x W x H)</label><input type="text" value={attributes.Dimensions || ''} onChange={e => handleAttributeChange('Dimensions', e.target.value)} placeholder="e.g. 20 x 15 x 10 cm" className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Material</label><input type="text" value={attributes.Material || ''} onChange={e => handleAttributeChange('Material', e.target.value)} placeholder="e.g. Cardboard, Plastic, Cotton" className="w-full border rounded-md px-3 py-2" /></div>
          <div><label className="block text-sm font-medium mb-1">Pack Size (Count)</label><input type="text" value={attributes.Count || ''} onChange={e => handleAttributeChange('Count', e.target.value)} placeholder="e.g. Pack of 50" className="w-full border rounded-md px-3 py-2" /></div>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold tracking-tight">Add Other Products</h1>
      </div>
