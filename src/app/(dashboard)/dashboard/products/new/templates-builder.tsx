...
const STEPS = [
  ...
  { id: 'downloads', label: 'Delivery Settings' },
...
const TEMPLATE_APPS = ['Notion', 'Excel', 'Google Sheets', 'Canva', 'Figma', 'PowerPoint', 'Google Slides', 'Word', 'Google Docs', 'PDF', 'Other']
const FORMATS = ['ZIP', 'PDF', 'XLSX', 'DOCX', 'PPTX', 'LINK']
...
export default function TemplatesProductBuilder({ productType }: { productType: string | null }) {
  ...
  // Delivery could be a file OR a URL (e.g. Notion template link, Canva template link)
  const [deliveryType, setDeliveryType] = useState('file') // 'file' or 'link'
  ...
  const { clearDraft } = useDraftAutoSave('templates', draftState, (data) => {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    if (deliveryType === 'file' && !uploadedFileId) {
      alert('Please wait for the template file to finish uploading to Google Drive.')
      return
    }

    if (deliveryType === 'link' && !templateLink) {
      alert('Please enter a valid template link.')
      return
    }
  ...
  const renderStep = () => {
    switch (currentStep) {
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Ultimate Minimalist Notion Dashboard"
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                placeholder="Describe your template, how it helps, and who it's for..."
        ...
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
              ...
              <p className="text-sm text-gray-500 mb-4">Upload a high-quality screenshot or preview of your template.</p>
              
              {!coverPhoto ? (
                <div 
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full aspect-video max-w-xl mx-auto border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-black hover:bg-gray-50 transition-colors"
                >
                  <ImageIcon className="w-10 h-10 text-gray-400 mb-3" />
                  <span className="text-sm font-medium text-gray-900">Click to upload preview</span>
                  <span className="text-xs text-gray-500 mt-1">16:9 ratio recommended (JPG, PNG, WebP)</span>
                </div>
              ) : (
        ...
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex gap-4 mb-8">
              <label className={`flex-1 flex flex-col p-4 border rounded-xl cursor-pointer transition-colors ${deliveryType === 'file' ? 'border-black bg-gray-50 ring-1 ring-black' : 'hover:bg-gray-50'}`}>
                <input type="radio" name="deliveryTypeRadio" checked={deliveryType === 'file'} onChange={() => setDeliveryType('file')} className="sr-only" />
                <span className="font-medium text-gray-900 mb-1">Upload a File</span>
                <span className="text-sm text-gray-500">ZIP, PDF, Excel, etc.</span>
              </label>
              <label className={`flex-1 flex flex-col p-4 border rounded-xl cursor-pointer transition-colors ${deliveryType === 'link' ? 'border-black bg-gray-50 ring-1 ring-black' : 'hover:bg-gray-50'}`}>
                <input type="radio" name="deliveryTypeRadio" checked={deliveryType === 'link'} onChange={() => setDeliveryType('link')} className="sr-only" />
                <span className="font-medium text-gray-900 mb-1">Secret Link</span>
                <span className="text-sm text-gray-500">Notion, Canva, Figma</span>
              </label>
            </div>
            ...
            {deliveryType === 'file' ? (
              <div className="text-center">
                {uploadedFileId ? (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-6">
                    ...
                    <p className="text-sm text-gray-600 mb-4">{templateFile?.name} ({formatFileSize(templateFile?.size || 0)})</p>
                    <div className="flex gap-3 justify-center">
                      <button type="button" onClick={removeTemplate} className="px-4 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100">
                        Remove & Re-upload
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="border border-gray-200 rounded-xl p-8">
                    ...
                    <p className="text-sm text-gray-500 mb-4">{templateFile?.name}</p>
                    ...
                    <p className="text-sm font-medium text-gray-700 mb-4">{uploadProgress}% • {formatFileSize(templateFile?.size || 0)}</p>
                  </div>
                ) : templateFile ? (
                  <div className="border border-gray-200 rounded-xl p-8">
                    <FileText className="w-12 h-12 text-[#FF7A00] mx-auto mb-4" />
                    <h4 className="text-lg font-medium text-gray-900 mb-2">{templateFile.name}</h4>
                    <p className="text-sm text-gray-500 mb-6">{formatFileSize(templateFile.size)} • Ready to upload</p>
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      <button type="button" onClick={removeTemplate} className="px-6 py-2 border border-gray-300 rounded-lg font-medium hover:bg-gray-50">Cancel</button>
                      <button type="button" onClick={uploadToGoogleDrive} className="px-6 py-2 bg-black text-white rounded-lg font-medium hover:bg-gray-800">Start Upload</button>
                    </div>
                  </div>
                ) : (
                  <div 
                  ...
                  >
                    ...
                    <h4 className="text-lg font-medium text-gray-900 mb-1">Upload template file</h4>
                    <p className="text-sm text-gray-500 mb-4">ZIP files are best if you have multiple items</p>
                    <span className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium shadow-sm mb-4">Choose File</span>
                    <p className="text-xs text-gray-400">Maximum: 200 MB</p>
                  </div>
                )}
              ...
              <div className="animate-in slide-in-from-top-2">
                <label className="block text-sm font-medium text-gray-900 mb-2">Secret Template Link *</label>
                <p className="text-sm text-gray-500 mb-4">Paste the share link. Buyers will receive this link automatically after purchasing.</p>
                <input
                  type="url"
                  value={templateLink}
                  onChange={e => setTemplateLink(e.target.value)}
                  placeholder="https://www.notion.so/your-template-id"
      ...
      case 3: // Product Details
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h3 className="text-lg font-medium mb-4">Template Specifics</h3>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-2">Software / App</label>
                <select
                  name="app"
                  value={app}
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Source File', 'Instructions PDF', 'Video Tutorial', 'Lifetime Updates', 'Support'].map(item => (
      ...
      case 4: // Pricing
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-3">Is this template free?</label>
      ...
      case 5: // Download Settings
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-3">Downloads / Access allowed per purchase</label>
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-lg p-4">
              ...
              <p className="text-sm text-orange-700">Customers will get immediate access to download the file or access the link directly from their order confirmation page after a successful payment.</p>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <div className="flex gap-6 p-6 border border-gray-200 rounded-xl bg-white shadow-sm">
              <div className="w-40 aspect-video flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                {coverPhoto && (
                  <img src={URL.createObjectURL(coverPhoto)} alt="Cover" className="w-full h-full object-cover" />
                )}
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900 mb-1">{name || 'Untitled Template'}</h3>
            ...
            <div className="bg-gray-50 rounded-xl p-6 space-y-4">
              <div className="flex items-center text-sm">
                ...
                <span className="text-gray-600 capitalize">{deliveryType}</span>
              </div>
              <div className="flex items-center text-sm">
                <Check className={`w-5 h-5 mr-3 ${(deliveryType === 'file' && uploadedFileId) || (deliveryType === 'link' && templateLink) ? 'text-green-500' : 'text-gray-300'}`} />
                <span className="font-medium w-32">Template:</span>
                <span className="text-gray-600">
                  {deliveryType === 'file' 
                    ? (uploadedFileId ? `${templateFile?.name}` : 'Missing File')
                    : (templateLink ? 'Link attached' : 'Missing Link')
                  }
                </span>
              </div>
            </div>
  ...
  const isNextDisabled = () => {
    ...
    if (currentStep === 2) {
      if (deliveryType === 'file') return !uploadedFileId
      if (deliveryType === 'link') return !templateLink
    }
    ...
    <div className="max-w-3xl mx-auto pb-24 pt-8 px-4 sm:px-0">
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">Add Template</h1>
